...
for itrThr = 34:34
  ...
  for stopThr = 23:23
    ...
    outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/HyInterpolate/6_points/out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ml_outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/BaseInterpolate/6_points/base_out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ...
    for seed = 101 : 150
          ...
          for ii = 1: no_of_iterations
            innerCount = 0;
            % I suppose points 2, 3, 8, 9, 16, and 20 are available
            %if ( configuration_to_predict == 12 )
              current_chunk = [operational_data_2_cleaned_inverted(ii, :) ; operational_data_3_cleaned_inverted(ii, :) ; (operational_data_8_1_cleaned_inverted(ii, :) + operational_data_8_2_cleaned_inverted(ii, :))/2 ; operational_data_9_cleaned_inverted(ii, :) ; operational_data_16_cleaned_inverted(ii, :) ; operational_data_20_cleaned_inverted(ii, :) ;];
            %endif
            ...
            do
               ...
               availableErrors = computeRE_6_points(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
               missingErrors = computeVE_6_points(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
            ...
            %==== ML =====
              ...
              ml_availableErrors = computeRE_6_points(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
             
              % The prediction is used to compute MRE of missing points using computeVE function
              ml_missingErrors = computeVE_6_points(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
