...
for itrThr = 34:34
  ...
  for stopThr = 23:23
    ...
    outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/HyInterpolate/3_points/out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ml_outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/BaseInterpolate/3_points/base_out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ...
    for seed = 101 : 150
          ...
          for ii = 1: no_of_iterations
            ...
            if ( configuration_to_predict == 10 )
              ...
              % Just three points, 20, 72, and 120, from real system contribute in machine learning phase             
              current_chunk = [operational_data_20_cleaned_inverted(ii, :) ; operational_data_72_cleaned_inverted(ii, :) ; operational_data_120_cleaned_inverted(ii, :) ; ]; 
            endif
            ...
            do
               ...
               availableErrors = computeRE_3_points(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
               missingErrors = computeVE_3_points(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
            ...
            %==== ML =====
              ...
              ml_availableErrors = computeRE_3_points(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
             
              % The prediction is used to compute MRE of missing points using computeVE function
              ml_missingErrors = computeVE_3_points(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
