...
for itrThr = 34:34
  ...
  for stopThr = 23:23
    ...
    outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/HyOptimumFindingVars/vars_out_wb_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ...
    for seed = 101 : 150
          ...
          for ii = 1: no_of_iterations
            ...
            do
               ...
               small_configurations = [1];
               small_scaled = ((1 ./ small_configurations) - mu_X) / sigma_X;
               [smallPred, ~, ~] = svmpredict ([1], small_scaled, model);
               small_value = smallPred(1); %* sigma_y + mu_y;
               
               big_configurations = [100000];
               big_scaled = ((1 ./ big_configurations) - mu_X) / sigma_X;
               [bigPred, ~, ~] = svmpredict ([1], big_scaled, model);
               big_value = bigPred(1); % * sigma_y + mu_y;
