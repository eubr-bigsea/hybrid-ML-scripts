...
for itrThr = 34:34
  ...
  for stopThr = 23:23
    ...
    outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/HyExtrapolate/108/out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ml_outPath = ["../plots/", query, "/", ssize, "/linear/missing_", num2str(opr_configurations(configuration_to_predict)), "/BaseExtrapolate/108/base_out_", num2str(itrThr), "_", num2str(stopThr), ".csv"];
    ...
    for seed = 101 : 150
          ...
          for ii = 1: no_of_iterations
            ...
            if ( configuration_to_predict == 10 )
              current_chunk = [operational_data_20_cleaned_inverted(ii, :) ; operational_data_40_cleaned_inverted(ii, :) ; operational_data_48_cleaned_inverted(ii, :) ; operational_data_60_cleaned_inverted(ii, :) ; operational_data_72_cleaned_inverted(ii, :) ; operational_data_80_cleaned_inverted(ii, :) ; operational_data_90_cleaned_inverted(ii, :) ; operational_data_100_cleaned_inverted(ii, :) ; ];
            endif
            ...
            do
               ...
               availableErrors = computeRE_120_108(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
               missingErrors = computeVE_120_108(predictions_linear{ii+1}, sigma_y, mu_y, avg_time_query_vector, configuration_to_predict);
            ...
            %==== ML =====
              ...
              ml_availableErrors = computeRE_120_108(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
             
              % The prediction is used to compute MRE of missing points using computeVE function
              ml_missingErrors = computeVE_120_108(ml_predictions_linear{ii+1}, ml_sigma_y, ml_mu_y, avg_time_query_vector, configuration_to_predict);
